...
__all__ = [
    ...
    "DDNPlusDistanceToGoalMeasurementConfig",
    "DDNPlusDistanceToOneGoalMeasurementConfig",
    "SuccessMeasurementConfig",
    "DDNPlusBasicSuccessMeasurementConfig",
    "DDNPlusPreferenceSuccessMeasurementConfig",
    "SPLMeasurementConfig",
    "DDNPlusBasicSPLMeasurementConfig",
    "DDNPlusPreferenceSPLMeasurementConfig",
    "SoftSPLMeasurementConfig",
    "DDNPlusSoftSPLMeasurementConfig",
    "DistanceToGoalRewardMeasurementConfig",
    "DDNPlusDistanceToGoalRewardMeasurementConfig",
    "DDNPlusDistanceToOneGoalRewardMeasurementConfig",
    # NAVIGATION LAB SENSORS
    "ObjectGoalSensorConfig",
    "DDNPlusGoalSensorConfig",
    "InstanceImageGoalSensorConfig",
    "InstanceImageGoalHFOVSensorConfig",
    "CompassSensorConfig",
    "DDNPlusCompassSensorConfig",
    "GPSSensorConfig",
...
@dataclass
class FindActionConfig(ActionConfig):
    r"""
    In DDNPlus Navigation tasks only, the find action is a discrete action. When called, the Agent
    will request to find object in Fov. 
...
@dataclass
class LeaveActionConfig(ActionConfig):
    r"""
    In DDNPlus Navigation tasks only, the find action is a discrete action. When called, the Agent
    will request to find object in Fov. 
...
@dataclass
class DDNPlusGoalSensorConfig(LabSensorConfig):
    r"""
    For Object Navigation tasks only. Generates a discrete observation containing
    the id of the goal object for the episode.
    ...
    :property goal_spec_max_val: If the `goal_spec` is OBJECT_ID, then `goal_spec_max_val` is the total number of different objects that can be goals. Note that this value must be greater than the largest episode goal category id.
    """
    type: str = "DDNPlusGoalSensor"
    goal_spec: str = "TASK_CATEGORY_LIST"
...
@dataclass
class DDNPlusCompassSensorConfig(LabSensorConfig):
    r"""
    For Navigation tasks only. The observation of the
    `EpisodicCompassSensor` is a single float value corresponding to
    the angle difference in radians between the current rotation of the robot and the
...
@dataclass
class DDNPlusBasicSuccessMeasurementConfig(MeasurementConfig):
    r"""
    For Navigation tasks only, Measures 1.0 if the robot reached a success and 0 otherwise.
    A success is defined as calling the `StopAction` when the `DistanceToGoal`
    Measure is smaller than `success_distance`.
    ...
    :property success_distance: The minimal distance the robot must be to the goal for a success.
    """
    type: str = "DDNPlusBasicSuccess"
    ddnplus_success_distance: float = 0.2
    ddnplus_find_times: int = 5


@dataclass
class DDNPlusPreferenceSuccessMeasurementConfig(MeasurementConfig):
    r"""
    For Navigation tasks only, Measures 1.0 if the robot reached a success and 0 otherwise.
    A success is defined as calling the `StopAction` when the `DistanceToGoal`
    Measure is smaller than `success_distance`.
    ...
    :property success_distance: The minimal distance the robot must be to the goal for a success.
    """
    type: str = "DDNPlusPreferenceSuccess"
    ddnplus_success_distance: float = 0.2
    ddnplus_find_times: int = 5
...
@dataclass
class DDNPlusBasicSPLMeasurementConfig(MeasurementConfig):
    r"""
    For Navigation tasks only, Measures the SPL (Success weighted by Path Length)
    ref: On Evaluation of Embodied Agents - Anderson et. al
    https://arxiv.org/pdf/1807.06757.pdf
    ...
    SPL = success * optimal_distance_to_goal / distance_traveled_so_far
    """
    type: str = "DDNPlusBasicSPL"


@dataclass
class DDNPlusPreferenceSPLMeasurementConfig(MeasurementConfig):
    r"""
    For Navigation tasks only, Measures the SPL (Success weighted by Path Length)
    ref: On Evaluation of Embodied Agents - Anderson et. al
    https://arxiv.org/pdf/1807.06757.pdf
...
@dataclass
class DDNPlusSoftSPLMeasurementConfig(MeasurementConfig):
    r"""
    For Navigation tasks only, Similar to SPL, but instead of a boolean,
    success is now calculated as 1 - (ratio of distance covered to target).
    SoftSPL = max(0, 1 - distance_to_goal / optimal_distance_to_goal) * optimal_distance_to_goal / distance_traveled_so_far
    """
    type: str = "DDNPlusSoftSPL"
...
class TopDownMapMeasurementConfig(MeasurementConfig):
    type: str = "TopDownMap"
    max_episode_steps: int = (EnvironmentConfig().max_episode_steps)  # TODO : Use OmegaConf II()
    map_padding: int = 3
...
@dataclass
class DDNPlusDistanceToGoalMeasurementConfig(MeasurementConfig):
    r"""
    In Navigation tasks only, measures the geodesic distance to the goal.

    :property distance_to: If 'POINT' measures the distance to the closest episode goal. If 'VIEW_POINTS' measures the distance to the episode's goal's viewpoint.
    """
    type: str = "DDNPlusDistanceToGoal"
    ddnplus_distance_to_goal: str = "POINT"


@dataclass
class DDNPlusDistanceToOneGoalMeasurementConfig(MeasurementConfig):
    r"""
    In Navigation tasks only, measures the geodesic distance to the goal.

    :property distance_to: If 'POINT' measures the distance to the closest episode goal. If 'VIEW_POINTS' measures the distance to the episode's goal's viewpoint.
    """
    type: str = "DDNPlusDistanceToOneGoal"
    ddnplus_distance_to_one_goal: str = "POINT"
...
@dataclass
class DDNPlusDistanceToGoalRewardMeasurementConfig(MeasurementConfig):
    r"""
    In Navigation tasks only, measures a reward based on the distance towards the goal.
    The reward is `- (new_distance - previous_distance)` i.e. the
    decrease of distance to the goal.
    """
    type: str = "DDNPlusDistanceToGoalReward"


@dataclass
class DDNPlusDistanceToOneGoalRewardMeasurementConfig(MeasurementConfig):
    r"""
    In Navigation tasks only, measures a reward based on the distance towards the goal.
    The reward is `- (new_distance - previous_distance)` i.e. the
    decrease of distance to the goal.
    """
    type: str = "DDNPlusDistanceToOneGoalReward"
...
class TaskConfig(HabitatBaseConfig):
    ...
    rank0_env0_measure_names: List[str] = field(default_factory=lambda: ["habitat_perf"])
    ...
    desired_resting_position: List[float] = field(default_factory=lambda: [0.5, 0.0, 1.0])
...
class SimulatorConfig(HabitatBaseConfig):
    ...
    scene_object_string_to_name: str = "default"
...
class LocobotConfig(HabitatBaseConfig):
    actions: List[str] = field(default_factory=lambda: ["base_actions", "camera_actions"])
    base_actions: List[str] = field(default_factory=lambda: ["go_to_relative", "go_to_absolute"])
    camera_actions: List[str] = field(default_factory=lambda: ["set_pan", "set_tilt", "set_pan_tilt"])
...
class PyrobotConfig(HabitatBaseConfig):
    ...
    robot: str = "locobot"
    sensors: Dict[str, PyrobotSensor] = field(default_factory=lambda: {
...
cs.store(
    package="habitat.task.actions.find",
    group="habitat/task/actions",
    name="find",
    node=FindActionConfig,
)
cs.store(
    package="habitat.task.actions.leave",
    group="habitat/task/actions",
    name="leave",
    node=LeaveActionConfig,
)
cs.store(
...
cs.store(
    package="habitat.task.lab_sensors.ddnpluscompass_sensor",
    group="habitat/task/lab_sensors",
    name="ddnpluscompass_sensor",
    node=DDNPlusCompassSensorConfig,
)
cs.store(
...
cs.store(
    package="habitat.task.lab_sensors.ddnplusgoal_sensor",
    group="habitat/task/lab_sensors",
    name="ddnplusgoal_sensor",
    node=DDNPlusGoalSensorConfig,
)
cs.store(
...
cs.store(
    package="habitat.task.measurements.ddnplus_distance_to_goal",
    group="habitat/task/measurements",
    name="ddnplus_distance_to_goal",
    node=DDNPlusDistanceToGoalMeasurementConfig,
)
cs.store(
    package="habitat.task.measurements.ddnplus_distance_to_one_goal",
    group="habitat/task/measurements",
    name="ddnplus_distance_to_one_goal",
    node=DDNPlusDistanceToOneGoalMeasurementConfig,
)
cs.store(
...
cs.store(
    package="habitat.task.measurements.ddnplus_distance_to_goal_reward",
    group="habitat/task/measurements",
    name="ddnplus_distance_to_goal_reward",
    node=DDNPlusDistanceToGoalRewardMeasurementConfig,
)
cs.store(
    package="habitat.task.measurements.ddnplus_distance_to_one_goal_reward",
    group="habitat/task/measurements",
    name="ddnplus_distance_to_one_goal_reward",
    node=DDNPlusDistanceToOneGoalRewardMeasurementConfig,
)
cs.store(
...
cs.store(
    package="habitat.task.measurements.ddnplus_basic_success",
    group="habitat/task/measurements",
    name="ddnplus_basic_success",
    node=DDNPlusBasicSuccessMeasurementConfig,
)
cs.store(
    package="habitat.task.measurements.ddnplus_preference_success",
    group="habitat/task/measurements",
    name="ddnplus_preference_success",
    node=DDNPlusPreferenceSuccessMeasurementConfig,
)
...
cs.store(
    package="habitat.task.measurements.ddnplus_basic_spl",
    group="habitat/task/measurements",
    name="ddnplus_basic_spl",
    node=DDNPlusBasicSPLMeasurementConfig,
)
cs.store(
    package="habitat.task.measurements.ddnplus_preference_spl",
    group="habitat/task/measurements",
    name="ddnplus_preference_spl",
    node=DDNPlusPreferenceSPLMeasurementConfig,
)
...
cs.store(
    package="habitat.task.measurements.ddnplus_soft_spl",
    group="habitat/task/measurements",
    name="ddnplus_soft_spl",
    node=DDNPlusSoftSPLMeasurementConfig,
)
